...
public class ContentProvidersActivity extends Activity {
	...
	private String[] getSongListing() {
		...
		Log.d(MyApplication.TAG, "song count = " + cursor.getCount());
		// Iterate over the Cursor, extracting each album name and song title.
		while (cursor.moveToNext()) {
			// Extract the song title.
			String title = cursor.getString(titleIdx);
			Log.d(MyApplication.TAG, " title = " + title);
			// Extract the album name.
			String album = cursor.getString(albumIdx);
			Log.d(MyApplication.TAG, " album = " + album);
	...
	private String[] getNames() {
		/**
		 * Listing 8-36: Accessing the Contacts Contract Contact Content
		 * Provider
		 */
	...
	private String performCallerId() {
		 ...
		 */
		String incomingNumber = "12345678";
		...
		Cursor cursor = getContentResolver().query(lookupUri, projection, null,
				null, null);
